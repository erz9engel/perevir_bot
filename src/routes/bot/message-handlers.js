...
const onCheckRequest = async (msg, bot) => {
    ...
    //Check similar
    let similarRequestsMessage = await checkSimilarRequests(msg.text);
    ...
    const sentActionMsg = await bot.sendMessage(process.env.TGMAINCHAT, '#pending' + similarRequestsMessage, options);
...
async function checkSimilarRequests(text){
    let message = '';
    const similarRequests = await Request.find(
        { $text: { $search: text }, fakeStatus: { $ne: 0 } },
        { score: { $meta: "textScore" } }
    ).sort({ score: { $meta: "textScore" } })
        .limit(3);

    const filteredRequests = similarRequests.filter(function (r) {
        return r._doc.score > 1.0;
    });

    if (filteredRequests.length > 0) {
        message = '\n\nCхожі звернення:';
        for (var i in filteredRequests) {
            if (filteredRequests[i].fakeStatus == 1) {
                message += `\n\n🟢 Правда:\n\n` + filteredRequests[i].text;
            } else if (filteredRequests[i].fakeStatus == -1) {
                message += `\n\n⛔ Фейк:\n\n` + filteredRequests[i].text;
            }
        }
    }
