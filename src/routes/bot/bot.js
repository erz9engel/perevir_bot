...
const onReplyWithComment = async (msg) => {
...
const onCheckRequest = async (msg) => {
    console.log(msg);
    ...
    await Request.findByIdAndUpdate(requestId, {moderatorMsgID: sentMsg.message_id, moderatorActionMsgID: sentActionMsg.message_id }, function(){});
}

const onUnsupportedContent = async (msg) => {
...
bot.on('message', async (msg) => {
    ...
    if (text === '/start') {
        ...
        await onCheckRequest(msg);
    } else if (msg.audio || msg.document || msg.voice || msg.location) {
        await onUnsupportedContent(msg);
    }
});
